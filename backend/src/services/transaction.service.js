...
class TransactionService {
  ...
  async fetchDeposits({ search, status, timeSlab, franchise, page=1, limit=10, startDate = null, endDate = null }) {
    const matchStage = {
        amount: { $gte: 0 },
        transactionStatus: status || TRANSACTION_STATUS.PENDING, // Ensure default status is applied
    };

    const endDateEnd = endDate ? new Date(new Date(endDate).setHours(23, 59, 59, 999)) : null;

    const startDateString = startDate ? new Date(startDate).toISOString() : "null";
    const endDateString = endDateEnd ? new Date(endDateEnd).toISOString() : "null";

    let CACHE_KEYS = {};
    const BASE_CACHE_KEY = `DEPOSITS_${(status || TRANSACTION_STATUS.PENDING).toUpperCase()}_${page}_${limit}_${timeSlab || "all"}_${franchise || "all"}_${startDateString}_${endDateString}`;
    
    CACHE_KEYS[`${BASE_CACHE_KEY}_DATA`] = `${BASE_CACHE_KEY}_DATA`;
    CACHE_KEYS[`${BASE_CACHE_KEY}_TIME_SLABS_COUNT`] = `${BASE_CACHE_KEY}_TIME_SLABS_COUNT`; // Corrected key name
    ...
    matchStage.requestDate = dateFilter.requestDate;
    ...
    // Moved this definition UP so it's available when timeSlabPipeline is constructed
    ...
    const timeSlabPipeline = [
        {
            $match: {
                ...
                ...(dateFilter.requestDate ? { requestDate: dateFilter.requestDate } : {}),
    ...
    if (status !== TRANSACTION_STATUS.PENDING && !search) {
        this.depositsCache.set(CACHE_KEYS[`${BASE_CACHE_KEY}_DATA`], responseData.data);
        this.depositsCache.set(CACHE_KEYS[`${BASE_CACHE_KEY}_TIME_SLABS_COUNT`], responseData.timeSlabCounts);
        this.depositsCache.set(CACHE_KEYS[`${BASE_CACHE_KEY}_TOTAL_PAGES`], responseData.totalPages);
        this.depositsCache.set(CACHE_KEYS[`${BASE_CACHE_KEY}_TOTAL_RECORDS`], responseData.totalRecords);
    }

    return responseData;
}

  async fetchWithdraws({ search, status, timeSlab, franchise, page, limit, startDate = null, endDate = null }) {
    const matchStage = {
      amount: { $lt: 0 },
    };
    ...
    let CACHE_KEYS = {};
    const BASE_CACHE_KEY = `WITHDRAWS_${(status || TRANSACTION_STATUS.PENDING).toUpperCase()}_${page}_${limit}_${timeSlab || "all"}_${franchise || "all"}_${startDateString}_${endDateString}`;
    ...
    // Apply date filter to matchStage
    matchStage.requestDate = dateFilter.requestDate;
