...
export const sendMessage = createAsyncThunk(
  'chat/sendMessage',
  async ({ conversationId, content, messageType = 'text', replyTo, mentions }: { conversationId: string; content: string; messageType?: 'text' | 'image'; replyTo?: string; mentions?: string[] }) => {
    return await chatService.sendMessage(conversationId, content, messageType, replyTo, mentions);
  }
);

export const sendImageMessage = createAsyncThunk(
  'chat/sendImageMessage',
  async ({ conversationId, imageUri, content, replyTo, mentions }: { conversationId: string; imageUri: string; content?: string; replyTo?: string; mentions?: string[] }) => {
    return await chatService.sendImageMessage(conversationId, imageUri, content, replyTo, mentions);
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendImageMessage.rejected, (state, action) => {
        state.error = action.error.message || 'Failed to send image';
      })
